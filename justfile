...
@build:
  npx slidev build --base=/kaggledays-functime
...
[private]
  ...
  #! /usr/bin/env zsh
  if ! gh repo list baggiponte | grep --quiet "/\bkaggledays-functime\b"; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create baggiponte/kaggledays-functime --private --source=.
              ;;
            [Nn])
              gh repo create baggiponte/kaggledays-functime --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:baggiponte/kaggledays-functime
          break
          ;;
        [Nn])
          break
